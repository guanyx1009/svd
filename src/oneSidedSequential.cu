...
#include "host_func.h" //for host functions
#include "cuda_error_check.cuh" //for error checking
#include <iostream>
#include <Eigen/Dense>
...
int main (int argc, char * argv[])
{
    ...
    float * matrix;
    int rows, columns;
    fillRowMajorOrderMatrix(&fp, &matrix, &rows, &columns); // malloc matrix memory
    fclose(fp);
...
//    std::cout << "Its right singular vectors are the columns of the thin V matrix:" << std::endl << svd.matrixV() << std::endl;
    ...
    // Column order matrix
    // malloc host_B host_U host_V memory
    createColumnMajorOrderMatrix(&host_B, matrix, rows, columns, &host_U, &host_V);
    // initialize host_AUX1 array to zero
    // initializeArray malloc memory
    ...
    float *host_U_res;
    float *host_V_res;
    initializeArray (&host_U_res, rows * rows);
    initializeArray (&host_V_res, columns * columns);
    descentOrdering(host_AUX1, columns, &host_U_res, &host_V_res, host_U, host_V, rows, columns);
